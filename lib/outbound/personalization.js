...
const SECTORS = [
  ...
  // Bare "industrial" fires on generalist boilerplate like "light industrial,
  // admin, professional" (a franchise's own self-description covering three
  // unrelated categories), which is a real false positive: it claims a
  // specific vertical from what is actually a "we do a bit of everything"
  // listing. Requiring a stronger phrase avoids that overclaim.
  [/\bmanufactur|\bindustrial (staffing|recruiting)\b|\bfactory\b|\bplant\b|\bproduction line\b/i, 'manufacturing and industrial'],
  ...
  [/\bsales\b/i, 'sales'],
  // Bare "office" is too broad -- it matches "franchise office" (a branch
  // location, said constantly in this dataset) as readily as it matches the
  // actual admin/clerical-work meaning. Require a real phrase instead.
  [/\bclerical|\badmin\b|office (staffing|support|admin)|front office|back office|receptionist|secretar\b/i, 'admin and clerical'],
...
// "light industrial, admin, professional" is a franchise's own generalist
// self-description -- three unrelated categories in one comma list, meaning
// "we cover a bit of everything," not "we specialize in X." Matching any one
// word from a list like that (whichever happens to sit first in SECTORS)
// produces a real overclaim: "focuses on manufacturing" or "focuses on
// admin" when the source text says neither, it says all three plus more.
// A short comma-enumeration anywhere in the text is treated as a generalist
// signal and blocks a vertical match entirely, regardless of which specific
// word would otherwise have hit.
const GENERALIST_LIST = /\b\w+(\s\w+){0,2},\s*\w+(\s\w+){0,2},\s*\w+(\s\w+){0,2}\b/;
...
export function verticalOf(note, industry, company) {
  ...
  if (GENERALIST_LIST.test(hay)) return null;
  const hits = [];
...
// A parenthetical after a company name is one of two very different things:
// a trading alias or acronym ("ComputerPeople (CP Staffing)", "Silver
// Recruiting of Virginia (SROVA)") which is noise and should drop, or a
// franchise BRANCH disambiguator ("Spherion Staffing & Recruiting
// (Mankato)") which is the one piece of information that makes a franchise
// lead distinct from every other location of the same brand -- dropping it
// collapses dozens of different offices into one identical, generic name in
// both the subject line and the personal opener.
//
// Heuristic: a short (<=3 word), capitalized token with no digits and no
// corporate-entity words reads as a place name, so it is kept, compacted to
// "Brand (Place)". Anything longer, lowercase, or alias-shaped is dropped as
// before.
const LOOKS_LIKE_A_PLACE = /^[A-Z][\w'.-]*(\s[A-Z][\w'.-]*){0,2}$/;
const ALIAS_WORDS = /\b(LLC|Inc|Ltd|Corp|Co|Staffing|Recruiting|Search|Agency|Group|Partners|Associates)\b/i;
...
export function companyShortOf(company) {
  ...
  if (paren) {
    ...
    if (LOOKS_LIKE_A_PLACE.test(inner) && !ALIAS_WORDS.test(inner) && !/^\d/.test(inner)) {
      return `${base} (${inner})`;
    }
  }
  return base || raw;
