...
import { specialtyOf, titlePhraseOf, companyShortOf } from './personalization';
...
export function variablesFor(lead) {
  ...
  const companyShort = companyShortOf(lead.company);

  // Every lead is in recruiting or staffing by the ICP lock, so specialtyOf
  // always has a true fallback ('recruiting') to land on. Unlike
  // personal_line below, this is safe to inline directly into a fixed
  // sentence: it is never blank.
  const spec = specialtyOf(lead.custom_note, lead.industry, lead.company) || 'recruiting';

  // null when the title is missing, a placeholder ("Contact"), or too long to
  // sit naturally in a sentence. Never substitute null/'' into a fixed
  // template fragment, hence step2_hook below is composed as one complete
  // sentence with its own branch for the no-title case, rather than the step
  // templates trying to interpolate this fragment directly.
  const titlePhrase = titlePhraseOf(lead.title);
  ...
  return {
    ...
    // Step 2's hook, so the whole sequence carries something specific to this
    // company, not just the opener. Composed here (not a raw {{title_phrase}}
    // fragment in the template) so a missing title never leaves a
    // grammatically broken gap like "running Company as  probably means...".
    step2_hook: titlePhrase
      ? `running ${companyShort} as ${titlePhrase} probably means the ${spec} pile lands squarely on you`
      : `running the ${spec} side of ${companyShort} probably means the applicant pile lands squarely on you`,
  };
}
