...
async function getOAuthClient(refreshToken) {
  ...
  if (!s.google_client_id || !s.google_client_secret) {
    throw new Error('Google credentials are not set up under Inboxes.');
  }
...
export async function scanForReplies() {
  ...
  let message = parts.length ? `Found ${parts.join(' and ')}.` : 'Nothing new since last check.';
  if (errors.length) message += ` (${errors.length} inbox had errors)`;

  return { success: true, repliedCount, bookedCount, message, errors: errors.length ? errors : undefined };
}
