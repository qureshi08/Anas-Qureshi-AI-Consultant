...
export async function processFollowUps({ limit = 200 } = {}) {
  ...
  for (const lead of leads) {
    // A lead sent with its own fully personalized copy (2026-08-17 rule) must
    // not fall back to the campaign's generic templated follow-up steps, that
    // would contradict the personalized first touch it already got. Skip it
    // here rather than auto-sending a boilerplate "Re: screening at..." —
    // its follow-up, if any, needs writing by hand, same as its first email.
    let hadCustomFirstTouch = false;
    if (lead.raw_data) {
      try {
        const parsed = JSON.parse(lead.raw_data);
        if (parsed.custom_subject && parsed.custom_body) hadCustomFirstTouch = true;
      } catch (_) {}
    }
    if (hadCustomFirstTouch) continue;
