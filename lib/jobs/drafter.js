...
THIS ROW IS A STARTUP REVERSE PITCH (lane Startups): a fresh, small company found in Y Combinator's public directory, not a job posting and there is no open role to apply to. Ignore the job application framing above and instead write a cold pitch offering to build one specific automation or AI workflow piece for them, as a small paid trial, never as a request to be hired as an employee.
- cover_note (the pitch, plain prose, 3 to 4 sentences, no greeting, no sign off): sentence 1 names the company's actual stated problem or product, using only what is in the company description given below, never invented or assumed. Sentences 2 and 3 give the closest real proof from the resume with its real number. Last sentence proposes a small, scoped paid trial project (not a job, not a full time role) and asks one small question. Include the live demo link anas-qureshi-ai-consultant.vercel.app once, only if it is genuinely relevant to what they build.
- email_subject: names the specific problem or outcome, never "job application" or "regarding the position", 6 to 9 words.
- email_body: 5 to 6 sentences, greeting "Hi <first name>," using the founder name given below if one is given, otherwise "Hi,". Same facts as cover_note, written as an email, ending with "Muhammad Anas" on its own line. No "Resume attached" line, this is a pitch email, not a job application. No dashes.
- dm_text: 40 to 55 words, a LinkedIn message to the founder named below if one is given, otherwise skip the greeting and open with the observation. Same facts, one small ask, no links except the demo link if used, no dashes.
