...
const SYSTEM = `You write job application copy for Anas Qureshi (legal name Muhammad Anas; sign emails "Anas"). He is applying for Applied AI Engineer / AI Engineer roles (and, for Gulf, fintech and ERP employers, AI integration and implementation engineer roles): the engineer who builds LLM features, integrations and backends into real products and client systems. Frame his real work that way, truthfully: he builds AI systems and integrations directly with business stakeholders. You are given his resume (the ONLY allowed source of facts about him) and one job posting. Return strict JSON with keys: fit_note, resume_variant, cover_note, dm_text, answers, resume_summary, project_order, skills_lead, email_subject, email_body, contact_role.
...
5. resume_variant: "se" (default, Applied AI Engineer), "ai" only when the posting is really an automation engineering role (n8n, Zapier, Make), "data" when it is mainly SQL, ETL, warehousing, dashboards.
...
8. Proof selection: the resume has several distinct proofs (banking and tax compliance integration with cryptographic signing (ZATCA), a multi tenant NestJS backend with Entra ID and row level security integrating two modeling engines, an AI recruitment portal screening resumes with Gemini, retail reporting automation and AI assisted matching, QA and UAT leadership for a US education data portal, recruiting workflow automation, credit risk model automation, his own outreach and AI assistant systems). Pick whichever ONE proof's domain most closely matches THIS company's actual stated business or problem, not simply the most impressive or most quantified one by default. A logistics or fleet company reads differently than a data infrastructure company or a recruiting company, match the story to them, do not reach for the same proof out of habit.`;
...
IF THE DESCRIPTION STARTS WITH "Open role:", A REAL, DETAILED REQUIREMENT EXISTS. This is the single most common mistake made on this task, so it is repeated here directly: DO NOT open the pitch with a sentence describing what the company does in general ("X is building/is a platform for/streamlines/automates Y..."). That sentence is the tagline, it sits below as background only, and using it as the opener has been sent out wrong before, confirmed on a real company (Lumari) where the requirement text plainly listed "Build AI agents from the ground up: designing orchestration logic, tool calling, prompting..." and "Power user of AI coding tools (Cursor, Claude Code, etc.)", a near perfect match, and the draft ignored all of it and opened with the tagline anyway. That is exactly the failure being corrected. Instead:
- Read the FULL ROLE REQUIREMENT block (Role Overview, Responsibilities, Requirements, Nice to haves) when present. Pick ONE real bullet or sentence from it, a named responsibility, a tool, a workflow, a phrase they actually used, and open with that, quoted or closely paraphrased. If it names a tool or practice the resume genuinely supports (for example "power user of AI coding tools" when the resume lists Antigravity, or a workflow automation responsibility when the resume shows n8n and LLM automation), say so plainly, that is the strongest possible opener because it is both specific and true.
- The skills are already split into "Of those, Anas genuinely has:" and "NOT on his resume, never claim these:". Only ever mention skills from the "genuinely has" list. If it says "none of the listed skills, this is a real stretch", say so plainly in one clause rather than pretending fit, and mark fit_note "Stretch:" or "Mismatch:".
- fit_note MUST name the specific requirement bullet or phrase the opener was built from (for example: "Fit: opener uses their 'power user of AI coding tools' requirement, matches his Antigravity experience"). A fit_note that claims the requirements were not detailed, when a FULL ROLE REQUIREMENT block was given, is itself a mistake, the text is right there, read it.
- If the visa line says "US citizen/visa only", note plainly in fit_note that a formal hire is closed but the small paid trial pitch, being independent contractor work, is not affected by that restriction, still worth sending.
- When there is no "Open role:" line at all, fall back to the company description: usually a real launch post, often with a founder's own words about a specific pain point, a number, an origin story, or a named tool or workflow. Find ONE concrete, checkable detail in it and build the opener around THAT. A sentence that only restates their tagline or mission statement is not personalization and is not acceptable even if true, because it could be sent to any company in their space unchanged. If the description genuinely has nothing concrete beyond a tagline, say so plainly in fit_note.
- Proof: pick exactly ONE resume proof whose domain most closely matches what THIS company actually does, per rule 8 above. Never stack two proofs from different domains in the same pitch. One well matched proof beats two generic ones.
- cover_note (the pitch, plain prose, 3 to 4 sentences, no greeting, no sign off): sentence 1 is the concrete requirement detail described above, never a tagline restatement. Sentences 2 and 3 give the one matched proof with its real number. Last sentence proposes a small, scoped paid trial project (not a job, not a full time role) and asks one small question. Include his LinkedIn profile linkedin.com/in/anasqureshiai once.
- email_subject: names the specific problem or outcome, never "job application" or "regarding the position", 6 to 9 words.
- email_body: short and scannable, never one dense block. Separate lines with a blank line between each: greeting "Hi <first name>," (using the founder name below if given, otherwise "Hi,") alone on its own line, then a short paragraph opening with the concrete requirement detail (not a tagline restatement), then a short paragraph with the one matched proof and its real number, then a single sentence paragraph proposing the small paid trial and asking one thing, then "Muhammad Anas" alone on its own line. 5 to 6 sentences total across those paragraphs. No "Resume attached" line, this is a pitch email, not a job application. No dashes.
- dm_text: Anas has no LinkedIn Premium, this is a connection request note, not a message, hard capped at 300 characters. 20 to 30 words maximum. "Hi <first name>," if the founder is named, then reference the same concrete requirement detail used in the email opener, in a few words, not a tagline restatement, then the barest hint of the pitch, nothing more, there is no room for proof or numbers here, that goes in the email. No links, no dashes.
 ...
 */
/* Groq models that write good cover notes, best first; each has its own rate limit (see the rotation in draftForJob). */
const PREFERRED_MODELS = ['openai/gpt-oss-120b', 'moonshotai/kimi-k2-instruct', 'llama-3.3-70b-versatile', 'qwen/qwen3-32b', 'openai/gpt-oss-20b', 'meta-llama/llama-4-maverick-17b-128e-instruct'];
const COOLDOWN = new Map();   // model -> time it may be tried again, set when Groq answers 429
/* Groq says "try again in 7m12.5s", "2.3s" or "750ms"; default one minute, cap one hour */
function retryAfterMs(msg) {
  const m = String(msg).match(/try again in ((?:\d+h)?(?:\d+m(?!s))?(?:[\d.]+(?:ms|s))?)/);
  if (!m || !m[1]) return 60000;
  let ms = 0; const t = m[1];
  const h = t.match(/(\d+)h/), mi = t.match(/(\d+)m(?!s)/), se = t.match(/([\d.]+)s$/), mss = t.match(/([\d.]+)ms$/);
  if (h) ms += h[1] * 3600000; if (mi) ms += mi[1] * 60000; if (mss) ms += Number(mss[1]); else if (se) ms += se[1] * 1000;
  return Math.min(Math.max(ms, 1000), 3600000);
}
...
export async function draftForJob(id) {
  ...
  async function ask(model, body, maxTokens, strict = true) {
    ...
    if (!res.ok) throw new Error(`Groq ${res.status} on ${model}${strict ? '' : ' (no strict mode)'}: ${(await res.text()).slice(0, 420)}`);
...
(posting text truncated)` : user;
  const errors = [];
  let out;
  // Rate limits (2026-09-26): one model on the free tier ran out after a handful of drafts and every retry hit the
  // same wall (55 of 128 jobs failed with Groq 429, the rest never got a turn). Each Groq model has its OWN limits,
  // so rotate through several strong ones and put a model on cooldown the moment it returns 429, instead of retrying it.
  let alive = null;
  try {
    const list = await fetch('https://api.groq.com/openai/v1/models', { headers: { Authorization: `Bearer ${key}` } });
    if (list.ok) alive = new Set(((await list.json()).data || []).map(m => m.id));
  } catch (e) { errors.push(`model list: ${e.message}`); }
  const ranked = PREFERRED_MODELS.filter(m => !alive || alive.has(m));
  const extra = alive ? [...alive].filter(id => !ranked.includes(id) && !/whisper|tts|guard|vision|embed|compound|prompt/i.test(id)) : [];
  for (const model of [...ranked, ...extra.slice(0, 2)]) {
    if ((COOLDOWN.get(model) || 0) > Date.now()) { errors.push(`${model} cooling down`); continue; }
    for (const [body, max, strict] of [[user, 2400, true], [shortUser, 2000, true], [shortUser, 2000, false]]) {
      try { out = await ask(model, body, max, strict); break; } catch (e) {
        errors.push(e.message);
        if (/Groq 429/.test(e.message)) { COOLDOWN.set(model, Date.now() + retryAfterMs(e.message)); break; }
        if (/Groq 404|decommissioned|does not exist/i.test(e.message)) break;
      }
    }
    if (out) break;
  }
  ...
  const variant = ['se', 'ai', 'data'].includes(out.resume_variant) ? out.resume_variant : 'se';
