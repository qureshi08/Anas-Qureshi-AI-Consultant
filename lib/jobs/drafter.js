...
- If the description below contains an "Open role:" line, THAT is what the pitch is about, not a generic automation or agent pitch. It names a real title, the skills they specifically asked for, salary, experience level, and visa terms, ALREADY split into "Of those, Anas genuinely has:" and "NOT on his resume, never claim these:". Reference the title, and only mention skills from the "genuinely has" list, never the "NOT on his resume" list, even if it would sound more impressive. Pick the resume proof that matches the genuinely-has skills, not just the company's general business. If "genuinely has" says "none of the listed skills, this is a real stretch", say so plainly in one clause in the pitch itself (the way a stretch is handled elsewhere in this prompt) rather than pretending fit where there is none, and mark fit_note "Stretch:" or "Mismatch:" accordingly. Never write a pitch that could apply to any AI company when a specific, detailed role requirement is sitting right there unused. If the visa line says "US citizen/visa only", note plainly in fit_note that a formal hire is closed but the small paid trial pitch, being independent contractor work, is not affected by that restriction, still worth sending.
