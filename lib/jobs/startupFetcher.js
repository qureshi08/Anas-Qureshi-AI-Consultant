 ...
 * a keyword match alone let a sales title through just for containing the word "agentic". */
function pickBestJob(jobs) {
  const eligible = jobs.filter(j => {
    const title = j.title || '';
    if (SENIOR_TITLE_RX.test(title)) return false;
    if (j.role && j.role !== 'eng' && NON_ENG_TITLE_RX.test(title)) return false;
    if (!j.role && NON_ENG_TITLE_RX.test(title)) return false;
    return experienceOk(j.minExperience);
  });
  if (!eligible.length) return null;
  const scored = eligible.map(j => {
    const blob = `${j.title || ''} ${(j.skills || []).join(' ')}`;
    return { job: j, score: STRONG.test(blob) ? 1 : 0 };
  });
...
export async function fetchAndStoreStartups({ budgetMs = 40000, maxDetails = MAX_DETAILS } = {}) {
  ...
  for (const c of candidates) {
    ...
    // Appended to description so the drafter sees it as part of what it already reads.
    const jobLine = job
      ? `\n\nOpen role: ${job.title}${job.roleSpecificType ? ` (${job.roleSpecificType})` : ''}, ${job.location || 'location unstated'}, ${job.salaryRange || 'salary unstated'}${job.equityRange ? `, ${job.equityRange}` : ''}, experience ${job.minExperience || 'unstated'}, visa: ${job.visa || 'unstated'}. Skills asked for: ${(job.skills || []).join(', ') || 'none listed'}.`
      : '';
