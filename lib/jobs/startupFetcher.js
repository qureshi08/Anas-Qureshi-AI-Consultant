...
async function companyDetail(slug, websiteDomain) {
  try {
    ...
    if (matched) return { founders: [matched.name], contact_url: matched.linkedin, email };
    if (email) return { founders: [], contact_url: founders[0]?.linkedin || null, email };
    if (founders.length) return { founders: [founders[0].name], contact_url: founders[0].linkedin, email: null };
