...
const SOFTWARE_TITLE_RX = /software|full.?stack|backend|back.end|frontend|front.end|data engineer|machine learning|\bml engineer\b|\bai engineer\b|platform engineer|devops engineer|product engineer|forward deployed|founding engineer|application engineer/i;
 ...
 * stack. Filters out roles above his real experience level, non-engineering roles, and any
 * visa-blocked role first, by the canonical role field when it exists, title text only as a
...
function pickBestJob(jobs) {
  const eligible = jobs.filter(j => {
    ...
    if (j.role) {
      ...
      // No canonical role field to trust at all: title must actively look wrong (NON_ENG) or,
      // when there are also no skills listed to check, actively look right (SOFTWARE_TITLE_RX),
      // never just "didn't look wrong" with zero positive signal either way.
...
export async function fetchAndStoreStartups({ budgetMs = 40000, maxDetails = MAX_DETAILS } = {}) {
  ...
  for (const c of candidates) {
    ...
    // One extra fetch, only for the one role actually being pitched, and only within budget.
    let jobDetailText = '';
    if (job?.url && Date.now() < deadline) jobDetailText = await fetchJobDetailText(job.url);
    ...
    const jobLine = job
      ...
      : '';
    // When a real role exists, it leads the description, the generic company blurb (often four
    // tagline-heavy paragraphs) trails as background only, kept short. Model testing found that
    // burying the job requirement at the end, after the company blurb, meant it got ignored, the
    // model just wrote from whatever it read first (a real observed failure on Lumari: the
    // requirement text was present and correct, the draft used the tagline anyway).
    const description = job
      ? `${jobLine}\n\n(Background only, do not open with this) About the company: ${(c.long_description || c.one_liner || '').slice(0, 600)}`
      : (c.long_description || c.one_liner || '');
