...
const NOT_HIS_LANE_RX = /\bml engineer\b|\bmle\b|machine learning engineer|research (engineer|scientist)|\bdata scientist\b|\bphd\b|\bintern(ship)?\b/i;
...
function pickBestJob(jobs) {
  const eligible = jobs.filter(j => {
    ...
    // YC's own canonical role type. "Founding SWE / MLE" and "AI/ML Harness Engineer" both
    // slipped past the title regex while typed "Machine learning" by YC (2026-09-24); the
    // abbreviation and the slash hybrid are not worth special casing when the source already
    // says which lane the role is in.
    if (/machine learning/i.test(j.roleSpecificType || '')) return false;
