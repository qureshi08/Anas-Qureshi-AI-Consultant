...
export async function fetchAndStoreStartups({ budgetMs = 40000, maxDetails = MAX_DETAILS } = {}) {
  ...
  for (const c of candidates) {
    ...
    rows.push({
      ...
      // Raised from 4000: the job's own requirement text (now appended last, the most specific
      // and valuable part) was getting truncated away by the old, tighter limit.
      description: (((c.long_description || c.one_liner || '') + jobLine)).slice(0, 6500),
      status: 'new',
