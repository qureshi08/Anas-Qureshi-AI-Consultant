...
async function companyDetail(slug, websiteDomain) {
  try {
    ...
    if (websiteDomain) {
      ...
      const matches = (html.match(rx) || []).filter(e => !/sentry|wixpress|noreply|no-reply/i.test(e));
      if (matches.length) email = matches[0].toLowerCase();
    }
    ...
    // allFounders is every paired founder found, for when the picked primary contact turns out
    // to be unreachable (locked profile, no Premium to connect) and a co-founder is a better
    // bet. Which LinkedIn profile is actually open to messages can only be seen by looking at
    // it, not scraped, so this hands over the real alternatives instead of hiding them.
    const allFounders = founders.map(f => ({ name: f.name, linkedin: f.linkedin }));
    if (matched) return { founders: [matched.name], contact_url: matched.linkedin, email, allFounders };
    if (email) return { founders: [], contact_url: founders[0]?.linkedin || null, email, allFounders };
    if (founders.length) return { founders: [founders[0].name], contact_url: founders[0].linkedin, email: null, allFounders };
    return { founders: [], contact_url: null, email: null, allFounders: [] };
  } catch {
    return { founders: [], contact_url: null, email: null, allFounders: [] };
  }
}
...
export async function fetchAndStoreStartups({ budgetMs = 40000, maxDetails = MAX_DETAILS } = {}) {
  ...
  for (const c of candidates) {
    ...
    let founders = [], contact_url = null, email = null, allFounders = [];
    if (details < maxDetails && Date.now() < deadline) {
      details++;
      ({ founders, contact_url, email, allFounders } = await companyDetail(c.slug, website));
    }

    // Other founders are real alternates if the picked primary contact's LinkedIn turns out
    // locked or unreachable (only visible by looking at the profile, never scrapeable), so hand
    // them over up front instead of leaving Anas stuck on a single unreachable pick.
    const others = allFounders.filter(f => f.name !== founders[0]);
    const altNote = others.length ? `Other founders, try if the primary contact is unreachable: ${others.map(f => `${f.name} (${f.linkedin})`).join(', ')}` : null;
