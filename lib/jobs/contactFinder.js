 ...
 */
async function findWebsite(company, jobUrl, knownWebsite) {
  // A website already known with certainty (e.g. the Startups lane's own domain, read straight
  // from Y Combinator's data at fetch time) is ground truth, never re-guessed by searching a
  // company name. Search-and-guess is the risky path: "item" (a real YC company) is also an
  // ordinary English word, so a search for "item official website" matched an unrelated page
  // and handed back a Hindustan Times contact address, a real, confirmed incident.
  if (knownWebsite) {
    try { return new URL(knownWebsite.startsWith('http') ? knownWebsite : `https://${knownWebsite}`).origin; } catch { /* fall through */ }
  }
  try {
...
/** Returns { email, website, note, otherEmails }. email is '' when nothing is published.
 * otherEmails is every other real on-domain address found, real alternates for when the picked
 * one bounces (a confirmed real failure, see feedback_no_domain_validation_before_send), never
 * silently discarded. knownWebsite, when given, is a website already known with certainty and
 * skips the risky search-by-name step. */
export async function findContactEmail({ company, url, knownWebsite }) {
  const site = await findWebsite(company, url, knownWebsite);
  if (!site) return { email: '', website: '', note: 'could not find the company website', otherEmails: [] };
  ...
  if (!emails.length) return { email: '', website: site, note: 'website found, no address published on it', otherEmails: [] };
  ...
  if (!onDomain.length) return { email: '', website: site, note: `website found, but no address on ${host}`, otherEmails: [] };

  const ranked = onDomain.sort((a, b) => {
    const rank = e => (GOOD_LOCAL.test(e.split('@')[0]) ? 0 : 1);
    return rank(a) - rank(b) || a.length - b.length;
  });
  return { email: ranked[0], website: site, note: '', otherEmails: ranked.slice(1) };
}
