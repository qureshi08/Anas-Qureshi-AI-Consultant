 ...
 *
 * Target relocked 2026-09-25 (lib/jobs/targets.js): AI / Solutions Engineer roles only, hard
 * gated on the title, in remote worldwide, the US, Canada / UAE / KSA and Pakistan.
 * Pay floor $1,500/mo where a salary is published. Pakistan and Gulf rows score +2.
 * LinkedIn's guest endpoint ignores its remote filter, so "remote" is added to the
 * Worldwide searches only.
 */
import { createAdminClient } from '../supabase/admin';
import { TARGET_TITLE_RX, LINKEDIN_KEYWORDS, LINKEDIN_LOCATIONS, SEED_BOARDS } from './targets';
...
// Applicant count on a posting's own page is the clearest signal of real competition.
// Above this, the odds of a cold application landing are low enough it is not worth a slot.
const MAX_APPLICANTS = 100;
const APPLICANT_DETAIL_BUDGET = 60;
...
const LOC_CLOSED = /\b(uk|united kingdom|england|london|europe|eu\b|germany|berlin|munich|hamburg|austria|switzerland|netherlands|amsterdam|france|paris|spain|portugal|italy|poland|ireland|dublin|sweden|denmark|norway|finland|latam|latin america|brazil|mexico|argentina|colombia|australia|new zealand|japan|korea|singapore|israel|hong kong|china|taiwan|vietnam|india|bengaluru|bangalore|hyderabad|mumbai|pune|chennai|delhi|noida|gurgaon|gurugram|thailand|indonesia|malaysia|philippines|south africa|nigeria|kenya|egypt|turkey|ukraine|romania|czech|hungary|greece|belgium|luxembourg|estonia|lithuania|latvia|bulgaria|serbia|peru|ecuador|bolivia|chile|uruguay|paraguay|venezuela|costa rica|guatemala|rio de janeiro|sao paulo|são paulo|bogot[aá]|lima|santiago|buenos aires|montevideo|quito|medell[ií]n|monterrey|guadalajara|mexico city|cdmx)\b/i;
// Canada removed 2026-09-22 (Anas wants Canada in the mix alongside UAE/KSA). Real risk: many
// "Remote - Canada" postings actually require in-country work authorization, which a cold
// Pakistan-based application cannot satisfy. locationOk lets these through; qualify by hand
// before spending a cover note if the posting reads as Canada-resident-only.
// US opened 2026-09-25 (Anas is open to relocating to the US): US rows pass and get lane 'US'.
const US_LOC = /\b(usa?|u\.s\.|united states|us only|americas|north america|nyc|new york|san francisco|sf bay|bay area|california|texas|austin|seattle|boston|chicago|denver|atlanta|los angeles|miami)\b/i;
...
async function feedLinkedin(days, deadline) {
  ...
  const tasks = [];
  // Page 0 of every search first, so a short time budget still covers every keyword and place.
  for (let p = 0; p < LINKEDIN_PAGES; p++) for (const loc of LINKEDIN_LOCATIONS) for (const kw of LINKEDIN_KEYWORDS) tasks.push({ loc, kw: loc === 'Worldwide' ? `${kw} remote` : kw, p });
...
/**
 * LinkedIn's public job page states applicant count in a few different spots depending on the
 * posting ("num-applicants__caption", or the plain "X applicants" / "Over 200 people clicked
 * apply" text). Best effort: null means unknown, never treated as competitive.
 */
async function linkedinApplicants(url) {
  try {
    const html = await get(url, { timeout: 10000 });
    const m = html.match(/"applicantCount"\s*:\s*"?(\d+)/)
      || html.match(/num-applicants__caption[^>]*>\s*([\d,]+)\s*applicant/i)
      || html.match(/Over\s+([\d,]+)\s+people clicked apply/i)
      || html.match(/([\d,]+)\s+applicants?\b/i);
    if (!m) return null;
...
function locationOk(j) {
  ...
  if (US_STATE.test(loc) || US_LOC.test(loc)) return true;
  if (LOC_CLOSED.test(loc)) return false;
  return true;
}
export function lane(location) {
  ...
  if (US_STATE.test(loc) || US_LOC.test(loc)) return 'US';
  return 'World';
}
function score(j) {
 ...
 */
export async function expireOldJobs({ postedDays = 4, seenDays = 5 } = {}) {
  ...
  // Anything Anas acted on is history and is never deleted. Skipped rows go immediately:
  // he already decided, and their address is remembered so they cannot reappear.
  const DELETABLE = ['new', 'shortlisted', 'expired'];
  const [{ data: aged }, { data: skipped }] = await Promise.all([
    admin.from('job_leads').select('id, key').in('status', DELETABLE).neq('source', 'platform').or(`posted_at.lt.${postedBefore},first_seen.lt.${seenBefore}`),
    admin.from('job_leads').select('id, key').eq('status', 'skipped'),
  ]);
  const stale = [...(aged || []), ...(skipped || [])];
  if (!stale.length) return { deleted: 0 };

  // Remember the addresses so a deleted posting cannot come back on the next fetch.
  await admin.from('job_seen_keys').upsert(stale.map(j => ({ key: j.key })), { onConflict: 'key', ignoreDuplicates: true });
  await admin.from('job_leads').delete().in('id', stale.map(j => j.id));
  return { deleted: stale.length };
}

/**
 * Clear out queued jobs that no longer fit the target (the AI Automation Engineer rows fetched
 * before the 2026-09-25 relock). Only untouched rows (new, shortlisted) in the main lanes;
 * anything Anas acted on stays. Their keys are remembered so they never come back.
 */
export async function purgeOffTarget() {
  const admin = createAdminClient();
  const { data } = await admin.from('job_leads').select('id, key, title').in('status', ['new', 'shortlisted']).not('lane', 'in', '(Training,Startups)').neq('source', 'platform');
  const off = (data || []).filter(j => !TARGET_TITLE_RX.test(j.title || ''));
  if (!off.length) return { purged: 0 };
  await admin.from('job_seen_keys').upsert(off.map(j => ({ key: j.key })), { onConflict: 'key', ignoreDuplicates: true });
  for (let i = 0; i < off.length; i += 200) await admin.from('job_leads').delete().in('id', off.slice(i, i + 200).map(j => j.id));
  return { purged: off.length };
}
...
export async function fetchAndStoreJobs({ days = 3, minScore = 3, linkedin = true, ats = true, budgetMs = 50000 } = {}) {
  ...
  const { data: knownBoards } = await admin.from('job_ats_boards').select('slug, ats');
  const knownSlugs = new Set((knownBoards || []).map(b => b.slug));
  const seeds = SEED_BOARDS.filter(([slug]) => !knownSlugs.has(slug)).map(([slug, ats]) => ({ slug, ats }));
  if (seeds.length) {
    const seedJobs = await feedAts(seeds, Math.min(deadline, Date.now() + 20000));
    const live = [...new Set(seedJobs.map(j => j.company))];
    if (live.length) await admin.from('job_ats_boards').upsert(seeds.filter(b => live.includes(b.slug)).map(b => ({ ...b, source: 'seed' })), { onConflict: 'slug', ignoreDuplicates: true });
  }
  ...
  const now = Date.now();
  const dropped = { stale: 0, title: 0, location: 0, pay: 0, score: 0, dupe: 0, buried: 0 };
  ...
  for (const j of raw) {
    ...
    if (!TARGET_TITLE_RX.test(j.title)) { dropped.title++; continue; }
  ...
  // Never re-add a posting that was deleted for being old.
  const { data: buried } = await admin.from('job_seen_keys').select('key');
  const buriedKeys = new Set((buried || []).map(b => b.key));
  let fresh = rows.filter(r => !buriedKeys.has(r.key));
  dropped.buried = rows.length - fresh.length;

  // Competition check: LinkedIn's own page states applicant count for some postings. Fewer
  // applicants is a real proxy for "max probability of being selected", so check it for the
  // best-scored fresh LinkedIn rows first, drop the crowded ones, and let the rest sort by it.
  dropped.competitive = 0;
  const byScore = fresh.filter(r => r.source === 'linkedin').sort((a, b) => b.score - a.score);
  let checked = 0;
  for (const r of byScore) {
    if (checked >= APPLICANT_DETAIL_BUDGET || Date.now() > deadline) break;
    checked++;
    const n = await linkedinApplicants(r.url);
    if (n === null) continue;
    r.applicants = n;
    if (n > MAX_APPLICANTS) { r.drop = true; dropped.competitive++; continue; }
    r.score += n < 20 ? 3 : n < 50 ? 1 : 0;
  }
  fresh = fresh.filter(r => !r.drop).map(({ drop, ...r }) => r);
  ...
  return { ok: true, raw: raw.length, passed: fresh.length, inserted, dropped, newBoards: newBoards.length, sources: stats, ms: Date.now() - started };
}
