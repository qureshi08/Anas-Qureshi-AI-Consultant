  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
        <a href={`/admin/jobs/all?status=work${laneQuery}`} className="mono" style={chip(view === 'work')}>To work &middot; {counts.new + counts.shortlisted}</a>
        {STATUSES.filter(s => s !== 'new' && s !== 'shortlisted').map(s => (
          <a key={s} href={`/admin/jobs/all?status=${s}${laneQuery}`} className="mono" style={chip(view === s, STATUS_COLOR[s])}>{STATUS_LABEL[s]} &middot; {counts[s]}</a>
        ))}
        <a href={`/admin/jobs/all?status=all${laneQuery}`} className="mono" style={chip(view === 'all')}>All &middot; {all.length}</a>
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
        <a href={`/admin/jobs/all?status=${view}`} className="mono" style={chip(!laneFilter, 'var(--ink3)')}>Every lane</a>
        {LANES.map(l => (
          <a key={l} href={`/admin/jobs/all?status=${view}&lane=${l}`} className="mono" style={chip(laneFilter === l, 'var(--forest)')}>{LANE_LABEL[l]} &middot; {rows.length && laneFilter === l ? rows.length : all.filter(j => j.lane === l && (view === 'all' || (view === 'work' ? ['new', 'shortlisted'].includes(j.status) : j.status === view))).length}</a>
        ))}
      </div>
